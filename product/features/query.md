...
3. Pipeline is executed against the `extraction_results` collection filtered by `extraction_id`
4. Results are displayed as a table

## Saved Aggregations

Users can save frequently-used queries as named aggregations:
- Create a saved aggregation with name, description, query text
- System generates and stores the MongoDB pipeline
...
- The `extraction_id` filter is always injected automatically — never part of the generated pipeline
